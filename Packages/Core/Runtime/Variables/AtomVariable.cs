...
namespace UnityAtoms
{
        ...
        where F : AtomFunction<T, T>
    {
        ...
        /// <summary>
        /// Used for Initialzing values from a save file
        /// </summary>
        static public Func<AtomBaseVariable, T, T> _loadFunc;
        static public Action<AtomBaseVariable> _addToResetListFunc;
        ...
        [SerializeField]
        public LoaderType _useLoader = LoaderType.Undefined;
        ...
        {
            if (_useLoader == LoaderType.Undefined)
            {
                Debug.LogError(name + " useLoader is Undefined. Please define it.");
            }

            if (_loadFunc == null && _useLoader == LoaderType.Enabled)
            {
                Debug.LogError(name + " has no loader function defined for type. Please define a save function or disable the UseLoader");
            }

            if (_useLoader == LoaderType.Enabled && _loadFunc != null)
            {
                _oldValue = _loadFunc.Invoke(this, InitialValue);
                _value = _oldValue;
            }
            else
            {
            ...
            if (addToResetList)
                _addToResetListFunc?.Invoke(this);
